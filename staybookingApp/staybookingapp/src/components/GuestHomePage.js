...
import {List, Tabs, message, Typography, Card, Carousel, Image, DatePicker, Button, Form, InputNumber} from "antd";
...
class SearchStays extends React.Component {
    ...
    search = async (query) => { //  query 封装了查询参数
        this.setState({
            loading: true,
        });
    ...
    render() {
        return (
            <List
                ...
                renderItem={(item) => (
                    <List.Item
                        actions={[<CancelReservationButton onCancelSuccess={this.loadData} reservationId={item.id} />,]}
                    >
                        <List.Item.Meta
